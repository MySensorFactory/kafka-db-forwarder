...
import com.factory.service.SensorDataService;
...
public class FactoryKafkaConfiguration {
    ...
    public void listen(@Payload final Pressure message, @Header(KafkaHeaders.RECEIVED_KEY) String key) {
        ...
        if (pressureMessageBuffer.isFull()) {
            ...
            debug(batch);
            sensorDataService.savePressureData(batch);
        }
    }
    ...
    public void listen(@Payload final Temperature message, @Header(KafkaHeaders.RECEIVED_KEY) String key) {
        ...
        if (temperatureMessageBuffer.isFull()) {
            ...
            debug(batch);
            sensorDataService.saveTemperatureData(batch);
        }
    }
    ...
    public void listen(@Payload final FlowRate message, @Header(KafkaHeaders.RECEIVED_KEY) String key) {
        ...
        if (flowRateMessageBuffer.isFull()) {
            ...
            debug(batch);
            sensorDataService.saveFlowRateData(batch);
        }
    }
    ...
    public void listen(@Payload final GasComposition message, @Header(KafkaHeaders.RECEIVED_KEY) String key) {
        ...
        if (gasCompositionMessageBuffer.isFull()) {
            ...
            debug(batch);
            sensorDataService.saveGasCompositionData(batch);
        }
    }
    ...
    public void listen(@Payload final NoiseAndVibration message, @Header(KafkaHeaders.RECEIVED_KEY) String key) {
        ...
        if (noiseAndVibrationMessageBuffer.isFull()) {
            ...
            debug(batch);
            sensorDataService.saveCompressorStateData(batch);
        }
    }
